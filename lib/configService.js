...
function processEnvironmentVariables() {
    var environmentVariables = [
        ...
        lwpubsubVariables = ['LWPUBSUB_AESKEY', 'LWPUBSUB_ALGORITHM'];
    ...
    /*----- LWPUBSUB variables -----*/
    if (anyIsSet(lwpubsubVariables)) {
        config.lwpubsub = {};
    }

    if (process.env.LWPUBSUB_AESKEY) {
      config.lwpubsub.aeskey = process.env.LWPUBSUB_AESKEY;
    } else { //assume aes-128-cbc as default
      config.lwpubsub.aeskey = constants.LWPUBSUB_AES128KEY;
    }

    if (process.env.LWPUBSUB_ALGORITHM) {
      config.lwpubsub.algorithm = process.env.LWPUBSUB_ALGORITHM;
    } else {
      config.lwpubsub.algorithm = 'aes-128-cbc';
    }
