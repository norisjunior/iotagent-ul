...
function generateTopics(callback) {
    ...
    topics.push('/99/+'); //lwpubsub - without attrs (MEASURES_SUFIX)
...
function generateCommandExecution(apiKey, device, attribute) {
    ...
    config.getLogger().debug(context, '\n\nCipher the payload! ALGORITHM: AES-CCM-128', ' KEY: ', config.getConfig().lwpubsub.aeskey);

    const ccm = require('aes-ccm');
    const crypto = require('crypto');

    var key = Buffer.from(constants.LWPUBSUB_KEY, 'hex');
    var random_nonce = Buffer.alloc(13);
    var send_nonce = crypto.randomFillSync(random_nonce)
    config.getLogger().debug('\n\n\nNonce: ', send_nonce);
    ...
    var res = ccm.encrypt(key, send_nonce, plaintext, aad_hex, tagLength);

    config.getLogger().debug('\n\n\nciphertext: ' + res.ciphertext.toString('hex'));
    config.getLogger().debug('\n\n\ntag: ' + res.auth_tag.toString('hex'));

    var finalPayload = Buffer.concat([send_nonce, res.auth_tag, res.ciphertext])
