...
function generateCommandExecution(apiKey, device, attribute) {
    ...
    config.getLogger().debug(context, '\n\nCipher the payload! ALGORITHM: %s KEY: %s',
        config.getConfig().lwpubsub.algorithm, config.getConfig().lwpubsub.aeskey);
    ...
    var cipher = crypto.createCipheriv(encryptAlgorithm, encryptKey, iv);
    ...
    config.getLogger().debug("\n\n\n\n\nPayload before zero padding text %s\n", payload);

    config.getLogger().debug("\n\n\n\n\nPayload before zero padding hex %s\n\n", Buffer.from(payload, 'hex'));

    payload = zeropad(payload, 8);

    config.getLogger().debug("\n\n\n\n\nPayload AFTER zero padding text %s\n", payload);

    config.getLogger().debug("\n\n\n\n\nPayload AFTER zero padding hex %s\n\n", Buffer.from(payload, 'hex'));

    var encryptedPayload = cipher.update(payload).toString('hex');
    //encryptedPayload = Buffer.concat([encryptedPayload, cipher.final()]);
    encryptedPayload += cipher.final();

    buf_res = Buffer.from(encryptedPayload, 'hex');
    config.getLogger().debug("Encrypted_text as buffer: ", buf_res);

    config.getLogger().debug("Encrypted result: ", encryptedPayload, "as string: ", buf_res.toString());
    ...
    if (Buffer.compare(encryptAlgorithm, 'no-sec') != 0) {
      if (Buffer.compare(encryptAlgorithm, 'aes-128-cbc') == 0) {
        lwpubsubFirstByte = Buffer.from('01', 'hex');
      } else if ((Buffer.compare(encryptAlgorithm, 'aes-192-cbc') == 0)) {
        lwpubsubFirstByte = Buffer.from('02', 'hex');
      } else if ((Buffer.compare(encryptAlgorithm, 'aes-256-cbc') == 0)) {
        lwpubsubFirstByte = Buffer.from('03', 'hex');
      } else if ((Buffer.compare(encryptAlgorithm, 'aes-128-ctr') == 0)) {
        lwpubsubFirstByte = Buffer.from('11', 'hex');
      } else if ((Buffer.compare(encryptAlgorithm, 'aes-192-ctr') == 0)) {
        lwpubsubFirstByte = Buffer.from('12', 'hex');
      } else if ((Buffer.compare(encryptAlgorithm, 'aes-256-ctr') == 0)) {
    ...
    finalPayload = lwpubsubFirstByte + iv + payload;

    config.getLogger().debug("\n\n\n\nlwpubsubFirstByte %s iv %s mqttPayload %s: \n\n\n\n", lwpubsubFirstByte, iv, payload);
