...
function generateCommandExecution(apiKey, device, attribute) {
    ...
    var cipher.setAutoPadding(false);
    ...
    config.getLogger().debug("\n\n\n\n\nPayload before zero padding text %s", payload);

    config.getLogger().debug("\n\n\n\n\nPayload before zero padding hex %s", Buffer.from(payload, 'hex'));

    payload = zeropad(payload, 8)

    config.getLogger().debug("\n\n\n\n\nPayload AFTER zero padding hex %s", Buffer.from(payload, 'hex'));
