...
function multipleMeasures(apiKey, device, message) {
    ...
    var AES128_key = Buffer.from(constants.LWPUBSUB_AES128KEY, 'hex'); //AES 128 Key
...
function messageHandler(topic, message, protocol) {
    var topicInformation = topic.split('/'),
        ...
        messageStr = message.toString(), //warning: need to decipher before to use .toString()
        parsedMessage;

    config.getLogger().debug('\n\nLWPubSub - IoT Agent - Message Handler\n\n');
    ...
    function processMessageForDevice(device, apiKey, topicInformation) {
        ...
        if (
            ...
            //Pensar quando for enviar comandos!
        ...
        } else if (topicInformation[2] && topicInformation[3] === null) {
            //GaD
            config.getLogger().debug('\n\nLWPubSub - IoT Agent - Message Handler - received - if(topicInformation[2] && topicInformation[3] === null)\n\n');
            multipleMeasures(apiKey, device, message);
        } else {
            config.getLogger().error(
                context,
                /*jshint quotmark: double */
