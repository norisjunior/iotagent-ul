...
function lwpubsubMessage(apiKey, device, message) {

  config.getLogger().debug('\n\n\n\nLWPubSub - IoT Agent - Message received\n\n');

  config.getLogger().debug('\n\nLWPubSub - RAW Received message [ %s ]\n', message);

  config.getLogger().debug("\n\nBuffer size: ", Buffer.byteLength(message));

  var message_received = Buffer.from(message, 'hex');

  config.getLogger().debug('\n\nLWPubSub - HEX Received message [ %s ]\n', message_received);

  var buf_res = 0;

  var is_encrypted = message_received.slice(0,1);
  const no_encryption = Buffer.from('30', 'hex');

  //Check encryption and decrypt message
  if (Buffer.compare(is_encrypted,no_encryption) != 0) {

    const ccm = require('aes-ccm');

    //LWPubSub Keys -- generate your own keys!
    var key = Buffer.from(constants.LWPUBSUB_KEY, 'hex'); //AES 128 Key

    var nonce = message_received.slice(0,13);
    config.getLogger().debug("\n\nNonce: ", nonce);

    var tag = message_received.slice(13,21);
    config.getLogger().debug("\n\nAuthTag (MIC): ", tag);

    var ciphertext = message_received.slice(21,Buffer.byteLength(message));
    config.getLogger().debug("\n\nCiphertext: ", ciphertext);

    //AAD: apiKey + "/" + device
    var aad = '/' + apiKey + '/' + device.id;
    config.getLogger().debug("\n\nAAD (preBuffer): ", aad);

    var aad_hex = Buffer.from(aad, 'utf8');
    config.getLogger().debug("\n\nAAD (posBuffer): ", aad_hex);

    const decrypt = ccm.decrypt(key, nonce, ciphertext, aad_hex, tag);
    buf_res = Buffer.from(decrypt.plaintext, 'hex');

    config.getLogger().debug('ok?: ' + decrypt.auth_ok);
    config.getLogger().debug('Decrypt as buffer: ' + buf_res);
    config.getLogger().debug('Decrypt as string: ' + decrypt.plaintext.toString('utf8'));

  } else {
    buf_res = message_received.slice(1,Buffer.byteLength(message));
  }



  // var AES192_key = Buffer.from(constants.LWPUBSUB_AES192KEY, 'hex'); //AES 192 Key
  // var AES256_key = Buffer.from(constants.LWPUBSUB_AES256KEY, 'hex'); //AES 256 Key
  ...
  //LWPubSub Encryption type
  // var message_received = Buffer.from(message, 'hex');
  // var encryption_mode = message_received.slice(0,1);
  // config.getLogger().debug("Encryption mode: ", encryption_mode);
  // var buf_res = 0;

  // Check encryption and decrypt message
  // if (Buffer.compare(encryption_mode,lwpubsub_aes_nosec) != 0) {
  //
  //     var aes_algorithm = '';
  //
  //     var key = '';
  ...
  //
  //     if (Buffer.compare(encryption_mode,lwpubsub_aes_cbc_128) == 0) {
  //         aes_algorithm = 'aes-128-cbc';
  //         key = AES128_key;
  //     } else if ((Buffer.compare(encryption_mode,lwpubsub_aes_cbc_192) == 0)) {
  //         aes_algorithm = 'aes-192-cbc';
  //         key = AES192_key;
  //     } else if ((Buffer.compare(encryption_mode,lwpubsub_aes_cbc_256) == 0)) {
  //         aes_algorithm = 'aes-256-cbc';
  //         key = AES256_key;
  //     } else if ((Buffer.compare(encryption_mode,lwpubsub_aes_ctr_128) == 0)) {
  //         aes_algorithm = 'aes-128-ctr';
  //         key = AES128_key;
  //     } else if ((Buffer.compare(encryption_mode,lwpubsub_aes_ctr_192) == 0)) {
  //         aes_algorithm = 'aes-192-ctr';
  //         key = AES192_key;
  //     } else if ((Buffer.compare(encryption_mode,lwpubsub_aes_ctr_256) == 0)) {
  //         aes_algorithm = 'aes-256-ctr';
  //         key = AES256_key;
  //     }
  //
  //     config.getLogger().debug("AES Algorithm: ", aes_algorithm, "key: ", key);
  //
  //     var iv = message_received.slice(1,17);
  //     config.getLogger().debug("IV: ", iv);
  //
  //     var encrypted = message_received.slice(17,Buffer.byteLength(message));
  //     config.getLogger().debug("Ciphered_text as buffer: ", encrypted);
  //
  //     var decipher = crypto.createDecipheriv(aes_algorithm, key, iv);
  //     decipher.setAutoPadding(false);
  //     var result_dec = decipher.update(encrypted).toString('hex');
  //     result_dec += decipher.final();
  //
  //     buf_res = Buffer.from(result_dec, 'hex');
  //     config.getLogger().debug("Deciphered_text as buffer: ", buf_res);
  //
  //     config.getLogger().debug("Deciphered result: ", result_dec, "as string: ", buf_res.toString());
  //
  // } else {
  //   buf_res = message_received.slice(1,Buffer.byteLength(message));
  // }
...
function messageHandler(topic, message, protocol) {
    ...
    config.getLogger().debug('\n\nLWPubSub - IoT Agent - Message Handler [ %s ]\n\n', message);
    ...
    function processMessageForDevice(device, apiKey, topicInformation) {
        ...
        } else if (topicInformation[2]) {
            //GaD
            config.getLogger().debug('\n\nLWPubSub - IoT Agent - Message Handler - received - if(topicInformation[2])\n\n');
            lwpubsubMessage(apiKey, device, message);
        } else {
            config.getLogger().error(
                context,
                /*jshint quotmark: double */
