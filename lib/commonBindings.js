...
function lwpubsubMessage(apiKey, device, message) {
  ...
  config.getLogger().debug('LWPubSub - Received message [ %s ]', message);

  config.getLogger().debug("Buffer size: ", Buffer.byteLength(message));
  ...
  var nonce = message_received.slice(0,12);
  config.getLogger().debug("\nNonce: ", nonce);

  var tag = message_received.slice(13,20);
  config.getLogger().debug("\nAuthTag (MIC): ", tag);
